...
const AuthModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center bg-black/60 backdrop-blur-sm pt-10 sm:pt-0">
            <div className="bg-[#111] border border-gray-800 rounded-2xl w-full max-w-md shadow-2xl relative overflow-hidden">
                <div className="bg-red-600 text-white text-[10px] font-bold py-1 px-4 text-center uppercase tracking-widest">
                    Live System: Version 2.2 (JWT/No-Cookies)
                </div>
                <div className="p-8">
                    <button
                        onClick={onClose}
                    ...
                    <h2 className="text-3xl font-bold text-white mb-2 text-center">
                        {isLogin ? 'Welcome Back v2.2' : 'Create Account v2.2'}
                    </h2>
                    <p className="text-gray-400 text-center mb-8">
                        {isLogin ? 'Enter details to sign in (JWT Mode)' : 'Sign up to start tracking (JWT Mode)'}
                    </p>
